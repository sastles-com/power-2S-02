...
| 電源 ON/OFF | **ディスクリートラッチ + EN 制御** (回路確定 2026-08-11 → [`docs/06`](docs/06-power-switch.md) §3.6)。**UX: 押下 130 ms で ON / 長押し 1.5 s で OFF** (単押しトグルはエッジトリガ必須のため不採用)。付帯要件: **電源状態 LED** のみ。強制オフ (CLR) 端子は設けない。**ESP32 ソフト電源断は一旦ペンディング** (2026-08-10、`H` ノードにパッドのみ予約) |
| オフ時消費電流 | **≤50 µA** (2026-08-11 確定)。MP1584 のシャットダウン電流 12〜20 µA が下限を決めるため ≤10 µA は不可能。実績見込みは **13 µA typ** |
...
- **NEVER** MP1584 の **EN ピンを 2S レール (6.0〜8.4 V) に接続しない** — **絶対最大 6 V**。
  ラッチはオープンドレインで引き下げるだけにする ([`docs/03`](docs/03-mp1584-module.md) §3.7)。
