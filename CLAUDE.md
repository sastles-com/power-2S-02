...
- **NEVER** BM (セル中点) ラインを省略しない。IP2326 のバランス端子は **BMS を経由せずセル中点へ**引く (legacy 踏襲)。
  ※「直結」は *BMS を迂回する* という意味と解釈。データシート標準回路は **RCB 100 Ω 直列**を要求するので
  これは入れる方針 (**ユーザー確認待ち** → [`docs/02`](docs/02-ip2326-module.md) §5 Q1)。
...
- ~~IP2326 のパッケージ/ピン配置・ISET 等の外付け定数、LCSC 入手性~~ → **解決** (2026-08-10、[`docs/02`](docs/02-ip2326-module.md) §0〜§3)。
  残るのは **RCB の可否 / DM,DP 未使用処理 / NTC を使うか** の 3 件 ([`docs/02`](docs/02-ip2326-module.md) §5)
