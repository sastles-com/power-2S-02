...
**充電段 (IP2326) と降圧段 (MP1584) は設計値まで確定。残るは電源スイッチのラッチ回路 → 作図着手。**
...
3. **ディスクリートラッチ回路の確定** ← **次はこれ。**
   リファレンス回路の選定と定数設計 ([`docs/06`](docs/06-power-switch.md) Q2)。
   **EN 仕様の確認は完了済み**なので着手できる。必須条件は [`docs/06`](docs/06-power-switch.md) §1.1:
   - EN は**オープンドレインで引き下げるだけ** (プルアップ・分圧を付けない。EN 絶対最大 6 V)
   - ⚠️ **電池挿入直後に「電源 ON」になってしまう** (EN floating = イネーブル) ので、
     **ラッチの電源投入時状態を OFF 側に確定させる回路が必要**
   - オフ時電流目標は **≤50 µA に改定** (MP1584 のシャットダウン 12〜20 µA が下限)
...
| **オフ時消費電流の目標** | ~~≤10 µA~~ → **≤50 µA** (MP1584 シャットダウン 12 µA typ / 20 µA max が下限。20 µA で 2000 mAh は 11 年) |
...
- **オフ時消費電流の目標値** — ~~≤10 µA~~ は **MP1584 のシャットダウン電流 12〜20 µA により達成不可能**と判明。
  **≤50 µA で妥協してよいか**の確認に変わった ([`docs/03`](docs/03-mp1584-module.md) §3.9 / [`docs/06`](docs/06-power-switch.md) Q1)
- **電池挿入直後に電源が入ってしまう挙動を許容するか** — MP1584 の EN は floating でイネーブルなので、
  ラッチに電源投入時 OFF を確定させる回路を追加しない限り「電池を挿すと点灯」になる
  ([`docs/06`](docs/06-power-switch.md) §1.1 要件 2)
