...
| 電源 SW 付帯機能 | ESP32 ソフト電源断 ○ / 電源状態 LED ○ / 強制オフ端子 × / ESP32 への状態通知 × |
...
- **ESP32 ソフト電源断のインターフェース仕様** (アクティブ Hi/Lo、パルス幅) ([`docs/06`](docs/06-power-switch.md) Q5)
- **コネクタ型番** (JST PH 系か。セル/BMS 系を多極 1 個に統合するか) ([`docs/01`](docs/01-legacy-analysis.md) Q1)
...
2. **バッテリ容量表記の矛盾**: 親 `docs/03-power-charging.md:47` の「LiPo 2000 mAh × 2 = 合計 4000 mAh」は**並列 (2P) 前提の表記**。2S 直列なら「2000 mAh @ 7.4 V」が正しい (エネルギー 14.8 Wh は同じ)。親側の記述修正が必要
3. **充電電流と AWG26 の整合**: 親 Q41 の指摘 (AWG26 安全電流 ~2 A) と IP2326 の充電電流設定を突き合わせる必要がある
