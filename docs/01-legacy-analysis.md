...
| 用途 | 信号 | legacy の対応 |
| --- | --- | --- |
| 充電入力 | PGND / CHARGE(5V) | J14 |
| セル 1 | B+ / BM | J5 |
| セル 2 | BM / B− | J6 |
| BMS 戻り | P+ / P−(PGND) | J1/J2 の一部 |
| 5V 出力 | PGND / 5V | J9 |

- **Type-C レセプタクルは載せない** (充電入力はコネクタのみ、[`docs/02`](02-ip2326-module.md))
- セル 1 / セル 2 / BMS 戻りを 1 個の多極コネクタに統合するかは型番選定時に判断 (§5 Q1)
...
| J7 | VOUT 6p | VOUT_SWITCHED が廃止されるため縮小 |
| J8 | VIN 6p | 充電/パック側を個別コネクタに分離 |
...
新規に必要なネット: ラッチ回路の内部ノード、**ESP32 ソフト電源断入力**、**降圧 IC の EN**、**LED 駆動**。

## 6. Open questions

- Q1: コネクタ型番の確定 (JST PH 系か、より小型か)。セル/BMS 系を多極 1 個に統合するか
